...
root._=lodash;}}).call(this); //The Facade Pattern - a pattern that hides the real complexity. addEventListener with cross-browser compatibility
...
function isSameArrayLength(arrayA,arrayB){return arrayA.length===arrayB.length;} //check if the objects are the same
...
function request(method,href,async){return new Promise((resolve,reject) => {var xmlhttp; //IE7+, Firefox, Chrome, Opera, Safari
if(window.XMLHttpRequest)xmlhttp=new XMLHttpRequest(); //IE5, IE6
else xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");xmlhttp.open(method,href,async);xmlhttp.onloadend=resolve;xmlhttp.onerror=reject;xmlhttp.send();});} //module pattern
//an array with all the created routes
var routes=[]; //a function to push a new route in to the routes array
function create(routeName,fn){routes.push(new Route(routeName,fn));} //a function that controles if user can go to that route
...
regexForLoop:/(^( )?(for|{|}))(.*)?/g};function init(obj){return new Promise((resolve,reject) => {if(!obj.container||!obj.template||!obj.data)return reject('The object has to have a container selector, a template string, and a data object');getDocElm(obj).then(parsTemplate(obj)).then(renderTemplate(obj)).then(obj => {resolve();}).catch(err => {reject(err);});});}function getDocElm(obj){return new Promise((resolve,reject) => {obj.container=getDocumentElement(obj.container);if(obj.container)return resolve(obj);return reject('Not a valid container selector!');});} //pars the template with the data given by the controller
function parsTemplate(obj){'use strict';return new Promise((resolve,reject) => {let match; //start the string with a declaration of an array
let code='var array = [];'; //hold the position of the 'mouse' on the template so that the computer knows where it is
...
code+=addToCode(slice(obj.template,currectPosition,match.index),false); //add een push to the code string with the variable
code+=addToCode(match.noRegex,true); //change the position of the 'mouse'
...
code+=addToCode(obj.template.substr(currectPosition,obj.template.length-currectPosition)); //add en return join so that the function gives back the string needed
code+='return array.join("");'; //remove all the linebreakers in the code
code=code.replace(/(\r\n|\n|\r)/gm,''); //populate the template with the string that is returned by the dynamic function 
obj.template=new Function('',code).call(obj.data);return resolve(obj);});}function renderTemplate(obj){obj.container.innerHTML=obj.template;}function addToCode(str,variable){return variable?str.match(settings.regexForLoop)?str+'\n':'array.push('+str+');':'array.push("'+str.replace(/"/g,'\\"')+'");';}var contentContainer;var kazStart={init:function(){this.cacheVars();this.addEvents();this.initRoutes();this.setStartLocation('home');},cacheVars:function(){contentContainer=document.getElementById('contentContainer');},addEvents:function(){addEvent(window,'hashchange',function(evt){go(evt.newURL.split('#')[1]);});},initRoutes:function(){create('home',controllers['home']);create('bestpractices',controllers['bestpractices']);create('bestpractices/{user}',controllers['bestpractices/{user}']);},setStartLocation:function(location){window.location.hash=location;go(location);}};var controllers={home:function(){request('get','../templates/home.html',true).then(response => {init({container:'#contentContainer',template:response.currentTarget.response,data:data});}).catch(function(error){console.error(error);});},bestpractices:function(){console.log(this);},'bestpractices/{user}':function(){var data={name:this.parameters.user,age:29};request('get','../templates/home.html',true).then(function(response){init({container:'#contentContainer',template:response.currentTarget.response,data:data});}).catch(function(error){console.error(error);});}};kazStart.init();